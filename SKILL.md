...
매입전표 등록:
```bash
tsx {baseDir}/scripts/register-expense.ts --date 2026-02-28 --amount 45000 --vendor 스타벅스강남점 --description "거래처 미팅 식대" --user 홍길동 --account-code 접대비 --card-company 신한카드 --approval-number 123456
```
...
3. 파싱 결과를 사용자에게 안내하고, **누가 썼고 무슨 용도인지** 물어보기
   - "스타벅스강남점에서 45,000원 결제되었습니다. 누가 썼고, 어떤 용도인가요?"
   - 예) "내가 썼고, 거래처 미팅 식대"
4. 사용자 답변에서 **사용자**(--user), **적요**(--description), **계정과목**(--account-code) 추출
   - 사용자: "내가" → 대표명, "홍길동이" → 홍길동
   - 용도 → 적요: "거래처 미팅 식대" → 계정과목 자동 추정 (식대 → 접대비/복리후생비)
5. 확인되면 register-expense.ts로 이카운트 매입전표 등록
6. 등록 결과 안내

**영수증 사진 증빉:**
1. 사용자가 영수증 사진을 메신저로 전달
2. LLM vision으로 영수증 이미지 분석 (가맹점명, 날짜, 품목, 금액 추출)
3. 추출된 정보를 안내하고, **누가 썼고 무슨 용도인지** 물어보기
4. 사용자 답변에서 사용자, 적요, 계정과목 추출
5. 확인되면 register-expense.ts로 이카운트 매입전표 등록
6. 등록 결과 안내
...
| 사용자 말 | 계정과목 |
|---|---|
| "식대", "밥", "커피", "배달" + 거래처/외부인 언급 | 접대비 |
| "식대", "밥", "커피", "배달" + 내부/팀 언급 | 복리후생비 |
| "택시", "버스", "주유", "주차", "톨게이트" | 여비교통비 |
| "사무용품", "토너", "종이", "펜" | 소모품비 |
| "책", "서적", "교육", "강의" | 도서인쇄비/교육훈련비 |
| "택배", "우편", "발송" | 운반비 |
| 판단 어려운 경우 | 사용자에게 직접 물어보기 |

> 계정과목은 사용자의 설명을 기반으로 추정하되, 불확실하면 반드시 다시 물어본다.
...
3. "스타벅스강남점에서 45,000원 결제되었습니다. 누가 썼고, 어떤 용도인가요?"
4. 사용자: "내가 썼고, 거래처 미팅 식대"
5. 사용자=대표, 적요="거래처 미팅 식대", 계정과목=접대비 으로 추정 → "대표님이 사용, 접대비로 등록할까요?"
6. 확인되면 register-expense.ts로 등록
7. "등록 완료! 날짜: 2/28, 사용자: 대표, 가맹점: 스타벅스강남점, 접대비, 공급가: 40,909원, 부가세: 4,091원"
